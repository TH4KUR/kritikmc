...
function ActiveRegistrationClosed({
  heading = "Registrations for Kritikmc are now over! 😭",
  message = ACTIVE_DEFAULT_CLOSED_COPY,
}) {
  return (
    <>
      <Nav />
...
export default async function ActiveRegistration() {
  if (!ACTIVE_ENABLED) {
    return (
      <ActiveRegistrationClosed
        heading="Active delegate registrations are currently closed."
  ...
  if (Date.now() > deadline.getTime() || Date.now() < start.getTime()) {
    return <ActiveRegistrationClosed />;
  }
